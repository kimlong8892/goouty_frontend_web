...
export const PaymentHistory: React.FC<PaymentHistoryProps> = ({ settlements }) => {
  ...
  if (settlements.length === 0) {
    if (isPWA) {
      return (
        <div className="p-10 text-center bg-slate-50/50 dark:bg-white/5 rounded-[24px] border border-slate-100 dark:border-white/5">
    ...
    <div className="space-y-4">
      {settlements.filter(settlement => settlement.amount > 0).map((settlement) => {
        ...
        return (
          ...
          >
            <div className={cn(isPWA ? "p-4" : "p-5 md:p-6", "space-y-6")}>
              {/* Status header */}
