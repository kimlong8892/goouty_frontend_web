              ...
              .map((member) => {
                ...
                return (
                  <div
                    key={member.id}
                    className={cn(
                      "flex items-center gap-2 p-3 rounded-2xl border-2 border-amber-200 dark:border-amber-900 bg-amber-50/50 dark:bg-amber-950/20",
                      isMobileView && "p-2 gap-2"
                    )}
                  >
                    <Avatar className={cn("border-2 border-amber-300 dark:border-amber-800 shadow-sm flex-shrink-0", isMobileView ? "w-10 h-10" : "w-16 h-16")}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={cn("flex mb-0.5", isMobileView ? "flex-col items-start gap-1" : "flex items-center justify-between")}>
                        <div className="flex items-center gap-1 min-w-0">
                          <span className={cn("font-bold text-slate-900 dark:text-slate-100 truncate", isMobileView ? "text-sm max-w-[100px]" : "text-lg")}>
                            {member.user.fullName || member.user.email.split('@')[0]}
                          </span>
                          {isCurrentUser && (
                            <span className="text-slate-400 font-medium text-[10px] flex-shrink-0">(bạn)</span>
                          )}
                        </div>
                        <Badge className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 hover:bg-amber-200 dark:hover:bg-amber-900/50 border-0 rounded-md px-1.5 py-0 text-[9px] font-bold shadow-none h-4 whitespace-nowrap">
                      ...
                      <div className="flex flex-col gap-0.5 overflow-hidden">
                        <div className="flex items-center gap-1 text-slate-500 dark:text-slate-400 text-[11px] min-w-0 overflow-hidden">
                          <Mail className="w-2.5 h-2.5 text-slate-400 dark:text-slate-500 flex-shrink-0" />
                          <span className="truncate max-w-[120px]">{member.user.email}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {isOwner && !isCurrentUser && (
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          className={cn(
                            "text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-full transition-colors",
                            isMobileView ? "h-8 w-8" : "h-10 w-10"
                          )}
                          title="Gửi lại lời mời"
                        >
                          <Send className={cn("h-5 w-5", isMobileView && "h-4 w-4")} />
                        </Button>
                        <Button
                          variant="ghost"
                          size="icon"
                          ...
                          className={cn(
                            "text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-full transition-colors",
                            isMobileView ? "h-8 w-8" : "h-10 w-10"
                          )}
                          title="Hủy lời mời"
                        >
                          <Trash2 className={cn("h-5 w-5", isMobileView && "h-4 w-4")} />
                        </Button>
                      </div>
                    )}
                  </div>
            ...
            {members
              ...
              .map((member) => {
                ...
                return (
                  ...
                  >
                    <Avatar className={cn("border-2 border-white dark:border-slate-800 shadow-sm flex-shrink-0", isMobileView ? "w-10 h-10" : "w-16 h-16")}>
                      <AvatarImage src={member.user.profilePicture} />
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={cn("flex mb-0.5", isMobileView ? "flex-col items-start gap-1" : "flex items-center justify-between")}>
                        ...
                        {isTripOwner && (
                          <Badge className="bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 hover:bg-orange-100 dark:hover:bg-orange-900/40 border-0 rounded-md px-1 py-0 text-[9px] font-bold shadow-none h-4 whitespace-nowrap flex-shrink-0">
