...
import { MapPin, Users, Calendar, DollarSign, Network, Search, ArrowRight, ArrowLeft } from 'lucide-react';
...
const DiagramComponent = ({ onSectionClick, activeSection }: DiagramComponentProps) => {
  const scatteredText = "Ý tưởng chuyến đi, thành viên nhóm, ngân sách và lịch trình - tất cả trong một nơi, có thể truy cập mọi lúc, mọi nơi.";
  const convergenceText = "Lập kế hoạch thông minh với các mẫu có sẵn. Goouty hiểu bạn cần gì và giúp tổ chức mọi thứ một cách hoàn hảo.";
  const organizedText = "Tìm kiếm theo địa điểm, ngày tháng, thành viên - bất cứ thứ gì bạn nghĩ đến đầu tiên. Tìm kiếm thông minh giúp bạn quản lý chuyến đi dễ dàng.";
      ...
      >
        <div className="relative w-24 h-24 md:w-32 md:h-32">
          ...
          <div className="absolute w-12 h-12 md:w-16 md:h-16 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary/30 flex items-center justify-center shadow-lg">
            <MapPin className="w-7 h-7 md:w-9 md:h-9 text-primary" />
          </div>
        </div>
      </div>
